"""End-to-end integration test: CSV → tool → connector → runtime → execution."""
...
TOOL_DEF = {
    "id": "list_products",
    "name": "List Products",
    "description": "Returns products filtered by optional category",
    "category": "READ",
    "source_ids": ["products"],
    "sql": (
        "SELECT id, name, price FROM products WHERE (:category IS NULL OR category = :category)"
    ),
    "parameters": [
        {
            "name": "category",
...
async def test_full_e2e_flow(tmp_path: Path) -> None:
    ...
    source = SourceConfig(id="products", name="Products CSV", type="file", url="products.csv")
    builder = ConnectorBuilder()
    builder.set_meta(name="Test Shop", version="1.0.0", slug="test-shop")
    connector_config = builder.build(sources=[source], tools=[tool], skills=[])
