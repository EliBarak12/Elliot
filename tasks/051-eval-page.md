...
- Fix suggestions inline (e.g. "Add a verb to the start of the description")

### `packages/mcp-plugin/src/tools/eval-tools.ts`
Two new MCP tools registered in `createElliotServer`:
- `elliot_run_eval`: `{ suiteId }` → load suite from `.elliot/eval/`, run via `runEvalSuite()`, save result, return `EvalRunResult`
- `elliot_quality_scan`: — → run `analyzeConnectorQuality()` on current session connector, return `ConnectorQualityScore`
